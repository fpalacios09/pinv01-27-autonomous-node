...
# Arduino Nano 33 BLE
# La Jetson NO compila el firmware. Arduino CLI carga directamente el
# archivo .ino.bin ya compilado y usa internamente el bossac de Arduino
# instalado por el core arduino:mbed_nano.
ARDUINO_CLI = os.getenv(
    "PINV_ARDUINO_CLI",
    shutil.which("arduino-cli") or "/usr/bin/arduino-cli"
)
ARDUINO_FQBN = os.getenv(
    "PINV_ARDUINO_FQBN",
    "arduino:mbed_nano:nano33ble"
)
ARDUINO_UPLOAD_TIMEOUT = int(
    os.getenv("PINV_ARDUINO_UPLOAD_TIMEOUT", "120")
)
...
def upload_arduino_bin(arduino_dir):
    """
    Busca un archivo .ino.bin dentro de la carpeta arduino/ y lo carga
    en un Arduino Nano 33 BLE mediante Arduino CLI.

    La Jetson NO compila el firmware. Arduino CLI realiza el reset de
    1200 bps, espera la reenumeración USB del bootloader y utiliza el
    bossac específico de Arduino instalado por arduino:mbed_nano.
    """
    ...
    if not (
        arduino_cli
        and os.path.isfile(arduino_cli)
        and os.access(arduino_cli, os.X_OK)
    ):
        detected_cli = shutil.which("arduino-cli")
        if detected_cli:
            arduino_cli = detected_cli
        else:
            print("[error] No se encontró arduino-cli.")
            print("[debug] Ejecutá primero scripts/install/install_nano_33_ble_uploader.sh")
            return False

    print(f"[debug] Arduino CLI detectado: {arduino_cli}")

    if not os.path.exists(MCU_PORT):
        print(f"[error] No existe el puerto MCU: {MCU_PORT}")
        print("[debug] Verificá la regla udev que crea /dev/mcu.")
        return False

    # /dev/mcu es la referencia estable. Arduino CLI recibe el tty real.
    # Durante el reset de 1200 bps, Arduino CLI detecta por sí mismo el
    # puerto temporal con el que reaparece el bootloader.
    real_mcu_port = os.path.realpath(MCU_PORT)

    if not os.path.exists(real_mcu_port):
        print(
            f"[error] El puerto real correspondiente a "
            f"{MCU_PORT} no existe: {real_mcu_port}"
        )
        return False

    print(f"[debug] Puerto MCU lógico: {MCU_PORT}")
    print(f"[debug] Puerto MCU real:   {real_mcu_port}")
    ...
    try:
        ...
        print("[error] Timeout consultando los cores de Arduino.")
        return False
    except Exception as e:
        print(f"[error] No se pudo ejecutar Arduino CLI: {e}")
        return False

    if core_result.returncode != 0:
        print("[error] No se pudo consultar los cores instalados de Arduino CLI.")
        if core_result.stderr:
            print(core_result.stderr)
        return False

    if "arduino:mbed_nano" not in core_result.stdout:
        print("[error] No está instalado el core Arduino Mbed OS Nano Boards.")
        print("[debug] Ejecutá primero scripts/install/install_nano_33_ble_uploader.sh")
        return False

    print("[debug] Core arduino:mbed_nano detectado.")
    print("[debug] Deteniendo procesos python en ejecución antes de subir firmware...")

    if not stop_running_code(exclude_pid=my_pid):
        print("[error] No se pudo detener correctamente el proceso Python activo.")
        return False
    ...
    print("[debug] Iniciando actualización del Nano 33 BLE...")
    print(f"[debug] FQBN: {ARDUINO_FQBN}")
    print(f"[debug] Binario: {bin_path}")
    ...
    try:
        upload_result = subprocess.run(
            ...
            stderr=subprocess.PIPE,
            universal_newlines=True,
            timeout=ARDUINO_UPLOAD_TIMEOUT
        )
    except subprocess.TimeoutExpired:
        print(
            f"[error] La carga del firmware superó "
            f"{ARDUINO_UPLOAD_TIMEOUT} segundos."
        )
        return False
    except Exception as e:
        print(f"[error] Excepción durante la carga del firmware Arduino: {e}")
        return False
    ...
    if upload_result.returncode != 0:
        print("[error] Falló la carga del firmware del Arduino Nano 33 BLE.")
        return False

    print("[debug] Firmware Nano 33 BLE cargado correctamente.")
    time.sleep(5)
    return True
...
def process_arduino_update(arduino_dir):
    ...
    - lee readme.txt
    - si dice true, busca el .ino.bin
    - lo carga en el Nano 33 BLE mediante Arduino CLI/BOSSA
    """
